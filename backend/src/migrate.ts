...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    `;
    if (!exists) {
      console.log("geo_places table missing — clearing stale migration records (keeping initial migration)...");
      try {
        // Keep the oldest record (0000) so its enum/type creation is not re-run.
        await sql`DELETE FROM drizzle.__drizzle_migrations WHERE created_at > (SELECT MIN(created_at) FROM drizzle.__drizzle_migrations)`;
      } catch {
